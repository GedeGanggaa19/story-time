      ...
      <div class="modal-content">    
        ...
        <div class="modal-body row">    
          ...
          <div class="password-section col-md-6">    
            ...
            <input type="password" v-model="formData.oldPassword" placeholder="Enter your old password" class="form-control mb-2 p-3" />    
    
            <label for="new-password" class="form-label">New Password</label>    
            <input type="password" v-model="formData.newPassword" placeholder="Enter your new password" class="form-control mb-2 p-3" />    
    
            <label for="confirm-password" class="form-label">Confirm New Password</label>    
            <input type="password" v-model="formData.confirmPassword" placeholder="Re-enter your new password" class="form-control mb-2 p-3" />    
          </div>    
        </div>    
...
export default {    
  setup() {    
    ...
    const user = ref({}); // State untuk menyimpan data pengguna    
    ...
    const imagePreview = computed(() => {    
      // Pastikan formData.value.image ada dan merupakan objek File    
      if (formData.value.image && formData.value.image instanceof File) {    
        return URL.createObjectURL(formData.value.image);    
      }    
      return user.value.image || defaultImage; // Kembali ke gambar default jika tidak ada    
    });    
    ...
    const userFetchData = async () => {    
      try {    
        ...
        // Update local user state dari store    
        user.value = authStore.user || {}; // Set to empty object if user is null    
    
        // Update formData with user data    
        formData.value.name = user.value.name || ''; // Default to empty string    
        formData.value.email = user.value.email || ''; // Default to empty string    
        formData.value.about = user.value.about || ''; // Default to empty string    
        formData.value.image = user.value.image || ''; // Default to empty string    
    
        // Update cookies dengan data terbaru    
        Cookies.set('user', JSON.stringify(user.value));    
      } catch (error) {    
        console.error('Error fetching user data:', error);    
        user.value = {}; // Reset user data on error    
        formData.value.name = '';    
    ...
    onMounted(async () => {    
      await userFetchData(); // Fetch user data saat komponen dimuat    
    });    
    ...
    const openWriteStoryModal = () => {    
      // Logic to open the write story modal    
    };    
...
.nav-tabs {    
  ...
  padding: 0; /* Ensure no padding is causing the line */    
